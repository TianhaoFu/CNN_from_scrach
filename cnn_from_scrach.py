...
logger = logging.getLogger('cnn_from_scrach')

# 训练数据
# path_minst_unpack = r'C:\Users\Tianh\Desktop\北邮\nn_base-master\data'
path_minst_unpack = r'data'
...
class MnistData(object):
    ...
    # 加载mnist
    def _load_mnist_data(self, kind='test'):
...
class ConvLayer(object):
    ...
    # 输入:
    #     x规格:
    #           x.ndim=4:原始规格,未padding batch * depth_i * row * col,  其中 depth_i为输入节点矩阵深度，
    #           x.ndim=3:x_col规格,已padding 前向:batch * (depth_i * filter_size * filter_size) * (out_size*out_size)
